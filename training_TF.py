...
from agent_TF import DeepQLearningAgent, mean_huber_loss
...
if(agent_type in ['DeepQLearningAgent']):
    ...
    if(supervised):
        ...
        model_iteration = 140000
        agent.load_model(file_path='models/{:s}'.format(version), iteration=model_iteration)
        print(f'Loaded model: {version} {model_iteration}')
