'''
script for training the PyToch based DQL agent 
'''
...
if(agent_type in ['DeepQLearningAgent']):
    ...
    decay = 0.97
    if(supervised and pre_trained_model):
        ...
        agent.load_model(file_path='models/{:s}'.format(version), iteration=trained_model_iteration)
        print(f'Loaded model: {version} {trained_model_iteration}')
